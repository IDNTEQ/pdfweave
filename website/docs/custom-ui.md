...
PDFweave internally uses [Ant Design](https://ant.design/).
You can apply the Ant Design theme to the PDFweave UI.
...
For supported labels, refer to [this i18n.ts](https://github.com/IDNTEQ/pdfweave/blob/main/packages/ui/src/i18n.ts).

PDFweave first loads `lang` from `options`, then reads and overrides from `options`' `labels`.
...
Create your own labels and use the i18n function from the plugin to retrieve the labels. Please refer to the official [Signature plugin](https://github.com/IDNTEQ/pdfweave/blob/main/packages/schemas/src/graphics/signature.ts) implementation in `@pdfweave/schemas`.

## UI Maximum Zoom Level

By default, PDFweave can zoom up to 200% of the original PDF size. 
