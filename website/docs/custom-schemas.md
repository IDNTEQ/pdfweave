...
By default, PDFweave allows you to use a text schema. However, some users may want to utilize schemas for images or QR codes.
These can be loaded as plugins from the `@pdfweave/schemas` package.

You can also create your own schemas and load them similarly as plugins.
This page explains how to use schemas from `@pdfweave/schemas` and how to create your own.

:::note
The default plugin registry used by `@pdfweave/generator` and `@pdfweave/ui` intentionally includes only the `text` schema. If your template uses any other built-in schema type, import it from `@pdfweave/schemas` and pass it through `plugins`.
:::

## Using Schemas from @pdfweave/schemas

Here, we explain how to import image, signature, and QR code schemas from `@pdfweave/schemas`.

First, install `@pdfweave/schemas`.

```bash
npm install @pdfweave/schemas
```

Next, import the required schemas from `@pdfweave/schemas` to `@pdfweave/generator` and `@pdfweave/ui`.

The following code shows an example of importing QR code, signature, and image schemas from `@pdfweave/generator` and `@pdfweave/ui`.
...
In this `@pdfweave/ui` example, we're using the Designer, but you can load plugins in the Form and Viewer in the same way.
...
By loading image, signature, and qrcode as plugins, you can render schemas of type image, signature, and qrcode found in your template's schemas.
...
If you have created a schema or have an idea for one, please share it on [GitHub Discussions](https://github.com/IDNTEQ/pdfweave/discussions/288).  
We believe that since PDFweave is developed as open-source, everyone should be able to share and develop schemas together.

### Overview of Custom Schemas / Plugins

Custom schemas consist of three elements, which are collectively referred to as plugins.  
The type definitions for plugins are defined within the [packages/common/src/types.ts](https://github.com/IDNTEQ/pdfweave/blob/main/packages/common/src/types.ts) file.
...
These libraries are manipulated through plugins to achieve their functionalities within PDFweave.  
...
- Demo: https://idnteq.github.io/pdfweave/
- Code: [packages/schemas/src/graphics/signature.ts](https://github.com/IDNTEQ/pdfweave/blob/main/packages/schemas/src/graphics/signature.ts)
...
PDFweave supports caching of memory or cpu-intensive content so that it can be re-used within the same rendering process.
...
Examples of caching are available in both [image](https://github.com/IDNTEQ/pdfweave/blob/main/packages/schemas/src/graphics/image.ts) and [barcode](https://github.com/IDNTEQ/pdfweave/blob/main/packages/schemas/src/barcodes/pdfRender.ts) schema render functions. You will need to choosing a caching key that captures the uniqueness of your generated PDF artifact (excluding attributes such as size and position, which are usually handled by pdf-lib on rendering). You will notice in the barcode schema that it requires more attributes to describe it's uniqueness compare to images which use the default `getCacheKey` function.

## Community Contributions

The PDFweave community has created and shared various custom plugins that you might find useful in your projects. Here are some community-contributed plugins:
...
- **Gist**: [A light weight qrcode plugin for PDFweave](https://gist.github.com/kyasu1/0def72d6f0826b0a9571b6e13f3c9065)
...
- **Features**:
  ...
  - Simple integration with PDFweave
...
:::tip
If you've created a useful plugin for PDFweave, please share it on [GitHub Discussions](https://github.com/IDNTEQ/pdfweave/discussions/288) so others can benefit from your work!
:::
